...
class MethodChannelFirebaseAuth extends FirebaseAuthPlatform {
  ...
  MethodChannelFirebaseAuth({required FirebaseApp app})
    ...
    // Windows: firebase_auth native code emits EventChannel events off the
    // platform thread, which Flutter rejects (auth-state / id-token). Skip
    // those listeners; imperative Auth APIs still work. See flutterfire#11933.
    final skipNativeListeners = !kIsWeb &&
        defaultTargetPlatform == TargetPlatform.windows;
    if (!skipNativeListeners) {
